...
\begin{align*}
  ...
  B &= \num{1.20 +- 0.43e3}.
\end{align*}
Die Halbwertsbreite (FWHM) enspricht der Breite des Graphen der Gaußfunktion bei halber Höhe. Analytisch kann die Halbwertsbreite als $\text{FWHM} = 2\sqrt{2 \text{ln}(2)} \cdot \sigma$ 
berechnet werden.
Mit dem zuvor ermittelten Wert für $\sigma$ ergibt sich der Wert 
\begin{equation*}
  \text{FWHM} = \qty{0.089 +- 0.001}{\degree}.
\end{equation*}
...
durchgeführt, wobei die Startwerte der freien Parameter auf zuvor berechnete und aus der Literatur bekannte Werte festgelegt werden. Ebenfalls wird der Wertebereich der Parameter 
um die zu erwartenden Größenordnungen eingeschränkt, um eine Konvergenz der Minimierung zu gewährleisten.
Die ermittelte Ausgleichsfunktion und die Messdaten sind in \autoref{fig:Reflek3} dargestellt. 
\begin{figure}
  \centering
...
\begin{align*}
  &\delta_\text{Poly} = \num{4.2e-6} & &\delta_\text{Si} = \num{1.6e-5} \\
  &\beta_\text{Poly} = \num{2.7e-8} & &\delta_\text{Si} = \num{9.8e-7} \\
...
$\alpha_\text{c, Poly} = \qty{0.32}{\degree}$ und $\alpha_\text{c, Si} = \qty{0.17}{\degree}$.
