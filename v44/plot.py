...
params, pcov = op.curve_fit(model, theta, counts, p0 = [np.mean(theta), np.std(theta), 175e3, 0])
...
plt.vlines([params[0] - 1/2*FWHM, params[0] + 1/2*FWHM] , 0, 180e3, color = "cornflowerblue", label = f"FWHM: {FWHM}°")
plt.legend()
plt.xlim(-0.3, 0.3)
plt.ylim(0, 1.8e5)
...
plt.plot(t2, c2, color = "firebrick", label = "Streureflektivität")
...
a_c = 0.153 # ° (kritischer Winkel von Si)
...
delta_Si = 6.5e-6 # 2. Schicht Silizium
...
def parratt(a, delta2, delta3, b2, b3, d2, sigma1, sigma2):
    ...
    n3 = 1.0 - delta3 + b3*1j
    a = np.deg2rad(a)
    kd1 = k *  np.emath.sqrt(n1**2 - np.cos(a)**2)
    kd2 = k * np.emath.sqrt(n2**2 - np.cos(a)**2)
    kd3 = k * np.emath.sqrt(n3**2 - np.cos(a)**2)
...
plt.plot(t, parratt(t, *params), color = "firebrick", alpha = .8, label = "Parrattalgorithmus")
#plt.vlines(0.153, 0, 10e3, label = r"$\alpha_c$ (Polysterol) = " + f"{a_c_Poly:.4f}°", color = "deeppink")
#plt.vlines(0.174, 0, 10e3, label = r"$\alpha_c$ (Si) = " + f"{a_c_Si:.4f}°", color = "rebeccapurple")
plt.legend()
plt.yscale("log")
plt.xlim(0, 2.5)
plt.ylim(None, 10e3)
